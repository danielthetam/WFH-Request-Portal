...
function Form() {
  ...
  const setRequester = (newName) => {
    request.current["requester"] = newName;
  }

  const setDuration = (newDuration) => {
    request.current["duration"] = newDuration;
  }
  ...
  const onSubmit = async (e) => {
    if (request.current["requester"] === "" || request.current["reason"] === "") {
      ...
      await axios.post("http://localhost:8080/api/requests", request.current);
...
function ActualDays({setDate, firstDay, numOfDays, currentDate}) {
  ...
  function newDate(event) {
    ...
    // TODO update state without re-render cuz it removes the black highlight when chosen
  }
  return (
    days.map((day) => {
      return (
